...
function actionMeta(action: AuditAction): {
  ...
  if (
    ["PATIENT_REGISTERED", "DOCTOR_REGISTERED", "LAB_REGISTERED"].includes(
      action,
    )
  )
    return {
...
const Dashboard: React.FC = () => {
  ...
  const [stats, setStats] = useState<StatCardProps[]>(() =>
    isAdminOrStaff ? INITIAL_STATS : INITIAL_STATS.slice(0, 3),
  );
  const [platformStats, setPlatformStats] =
    useState<Product[]>(INITIAL_PLATFORM);
      ...
      prev.map((item) =>
        item.name === "Pending Verifications"
          ? {
              ...
              revenue: pendingCounts.total > 0 ? "Awaiting" : "Clear",
              trend: pendingCounts.total > 0 ? "down" : "up",
            }
          : item,
      ),
    );
      ...
      setPlatformStats((prev) =>
        prev.map((item) => {
          if (item.name === "Active Patients")
            return { ...item, sales: patients.length };
          if (item.name === "Verified Doctors")
            return { ...item, sales: doctors.length };
          if (item.name === "Verified Labs")
            return { ...item, sales: labs.length };
          return item;
        }),
      );
    } catch {
            ...
            {!activityLoading &&
              ...
              recentActivity.slice(0, 5).map((log) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${bg} ${iconColor}`}
                    >
                      {icon}
                    </div>
            ...
            {[
              ...
              <div key={row.label}>
                ...
                <div className="w-full bg-slate-100 rounded-full h-2">
                  <div
                    className={`bg-gradient-to-r ${row.gradient} h-2 rounded-full`}
                    style={{ width: `${row.pct}%` }}
                  />
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    {
                      label: "Registrations",
                      value: recentActivity.filter((l) =>
                        l.action.includes("_REGISTERED"),
                      ).length,
                      color: "text-blue-600",
                    },
                    {
            ...
            <tbody>
              {platformStats.map((stat) => (
                ...
                >
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-emerald-100 to-teal-100 rounded-lg flex items-center justify-center">
                        {stat.name.includes("Patient") && (
                          <Users className="w-5 h-5 text-emerald-600" />
                        )}
                        {stat.name.includes("Doctor") && (
                          <UserCheck className="w-5 h-5 text-blue-600" />
                        )}
                        {stat.name.includes("Lab") && (
                          <Activity className="w-5 h-5 text-purple-600" />
                        )}
                        {stat.name.includes("Staff") && (
                  ...
                  <td className="py-4 px-4">
                    <span
                      className={`inline-flex px-2 py-1 rounded-md text-xs font-medium ${stat.revenue === "Online" || stat.revenue === "Active" ? "bg-emerald-100 text-emerald-700" : "bg-orange-100 text-orange-700"}`}
                    >
                      {stat.revenue}
                    </span>
                  </td>
