...
export const InfoRow: React.FC<InfoRowProps> = ({ icon: Icon, label, value, className = "" }) => {
    ...
    return (
        ...
        >
            <Icon className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: "#f47920" }} />
            <div className="flex-1 min-w-0">
                <p className="text-xs font-medium mb-0.5" style={{ color: "#555555" }}>{label}</p>
                <p className="text-sm font-medium break-words" style={{ color: "#1a3c6e" }}>{value}</p>
            </div>
        </div>
    );
};