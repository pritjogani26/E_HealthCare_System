...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`
        fixed top-0 left-0 h-screen bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950
        border-r border-slate-800/50 z-50 transition-all duration-300
        ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
        w-72 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-900
      `}
      >
        {/* Logo */}
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-3 py-2 rounded-md transition-all duration-200 ${subActive
                                ? "text-emerald-400 bg-slate-800/50 font-medium"
            ...
            {supportItems.map((item, idx) => {
              ...
              return (
                ...
                `}
                  >
                    <div className="flex items-center gap-3">
                      <item.icon className={`w-5 h-5 ${active ? "text-emerald-400" : ""}`} />
                      <span className="font-medium text-sm">{item.label}</span>
                    </div>
            ...
            <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-xl p-4">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="font-semibold text-emerald-400">156</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>Available Beds</span>
                  <span className="font-semibold text-emerald-400">12/50</span>
                </div>
              </div>
            </div>
          </div>
