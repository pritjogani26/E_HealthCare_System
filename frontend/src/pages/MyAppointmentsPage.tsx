...
import { Calendar, Clock, RefreshCw, FileText } from "lucide-react";
import { handleApiError } from "../services/api";
import { cancelAppointment, getMyAppointments } from "../services/doctor_api";
import { getPrescriptionByAppointment } from "../services/prescription_api";
...
const MyAppointmentsPage: React.FC = () => {
  ...
  const handleViewPrescription = async (appointmentId: number) => {
    setViewingPrescriptionId(appointmentId);
    try {
      const presc = await getPrescriptionByAppointment(appointmentId);
      if (presc.pdf_url) {
        ...
        >
          ...
          >
            ...
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.background = "#e8f0f7")
              }
              onMouseLeave={(e) =>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            ...
                            <span style={{ margin: "0 4px", color: "#d0dff0" }}>
                              |
                            </span>
                      ...
                      >
                        ...
                        {/* View Prescription (completed appointments) */}
                        {apt.status === "completed" && (
                          <button
                            onClick={() => handleViewPrescription(apt.appointment_id)}
                            disabled={viewingPrescriptionId === apt.appointment_id}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "5px",
                              ...
                              color: "#2563eb",
                              cursor:
                                viewingPrescriptionId === apt.appointment_id
                                  ? "not-allowed"
                                  : "pointer",
                              fontWeight: 600,
                              fontSize: "12px",
                              opacity:
                                viewingPrescriptionId === apt.appointment_id ? 0.6 : 1,
                              transition: "background 0.15s",
                              whiteSpace: "nowrap",
                            }}
                            onMouseEnter={(e) => {
                              if (viewingPrescriptionId !== apt.appointment_id)
                        ...
                        {/* Cancel (confirmed/pending future appointments) */}
                        {(() => {
                          const isCancellableStatus =
                            apt.status.toLowerCase() === "confirmed" ||
                            apt.status === "pending";
                          let isFuture = true;
                          if (apt.slot_date && apt.start_time) {
                            isFuture =
                              new Date(`${apt.slot_date}T${apt.start_time}`) >
                              new Date();
                          }
                          return (
                            isCancellableStatus &&
