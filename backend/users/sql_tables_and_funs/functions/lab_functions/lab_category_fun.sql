...
-- =============================================================
--  Functions
--    1. create_lab_test_category   → INSERT
--    2. update_lab_test_category   → UPDATE
--    3. delete_lab_test_category   → soft-delete (is_active = false)
--    4. list_lab_test_categories   → SELECT (with optional filters)
-- =============================================================
...
/*
USAGE:
    SELECT * FROM public.create_lab_test_category(
        'Haematology',
...
-- Raises an exception if the category does not exist or is
...
USAGE — update name only:
    SELECT * FROM public.update_lab_test_category(
        p_category_id   => 1,
...
/*
USAGE:
    SELECT * FROM public.delete_lab_test_category(
        1,
        'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'::uuid
    );
*/
...
--   p_is_active       NULL = all rows, TRUE = active only,
--                     FALSE = inactive only
...
AS $$
BEGIN
    -- Clamp limit to 100 to prevent abuse
...
/*
USAGE — active categories, first page:
    SELECT * FROM public.list_lab_test_categories();
