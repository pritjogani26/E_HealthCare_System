...
BEGIN
    ...
    IF a_status NOT IN ('SUCCESS', 'FAILURE') THEN
        RAISE EXCEPTION 'AUDIT_LOG_INVALID_STATUS';
    END IF;

    INSERT INTO public.audit_logs (
        performed_by_id, target_user_id, action,
        entity_type, details, status
    )
    VALUES (
        a_performed_by_id, a_target_user_id, a_action,
        a_entity_type, a_details, a_status
    );
END;
$$;
...
AS $$
BEGIN
    IF a_limit IS NULL OR a_limit <= 0 OR a_limit > 1000 THEN
        a_limit := 100;
    END IF;
