"""
Doctor serializers — pure DRF Serializer (no ModelSerializer).
"""
...
class _QualWriteSerializer(serializers.Serializer):
    ...
    year_of_completion = serializers.IntegerField(required=False, allow_null=True, min_value=1900)


class _SpecWriteSerializer(serializers.Serializer):
    specialization_id  = serializers.IntegerField()
    is_primary         = serializers.BooleanField(default=False)
    years_in_specialty = serializers.IntegerField(required=False, allow_null=True, min_value=0)
...
class DoctorProfileSerializer(serializers.Serializer):
    ...
    def get_qualifications(self, obj):
        return QualReadSerializer(self._d(obj).get("qualifications", []), many=True).data

    def get_specializations(self, obj):
        return SpecReadSerializer(self._d(obj).get("specializations", []), many=True).data
...
class DoctorRegistrationSerializer(serializers.Serializer):
    ...
    experience_years   = serializers.DecimalField(max_digits=6, decimal_places=2, default=Decimal("0.00"), min_value=0)
    consultation_fee   = serializers.DecimalField(max_digits=10, decimal_places=2, required=False, allow_null=True, min_value=0)
...
class DoctorProfileUpdateSerializer(serializers.Serializer):
    ...
    experience_years    = serializers.DecimalField(max_digits=6, decimal_places=2, required=False, min_value=0)
...
class DoctorListSerializer(serializers.Serializer):
    ...
    def get_specializations(self, obj):
        d = obj if isinstance(obj, dict) else {}
        return SpecReadSerializer(d.get("specializations", []), many=True).data